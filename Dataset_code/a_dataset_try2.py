    ...
    for idx, qa in enumerate(raw_qas):
        ...
        for item in evidence:
            if isinstance(item, str):
                ...
                document = "Context 문서"
                field = quote.split(":", 1)[0].strip() if ":" in quote else "본문"
            elif isinstance(item, dict):
                quote = safe_text(item.get("quote"), 1500)
                document = safe_text(item.get("document"), 500) or "Context 문서"
                field = safe_text(item.get("field"), 200) or "본문"
            else:
                continue
            if quote and quote in bundle.context:
                verified.append({
                    "document": document,
                    "field": field,
                    "quote": quote,
...
def training_completion(qa: dict[str, Any]) -> str:
    ...
    if qa["evidence"]:
        for item in qa["evidence"]:
            lines.append(f"- {item['document']} | {item['field']}: {item['quote']}")
    else:
...
def main() -> None:
    ...
    parser.add_argument("--questions-per-document", type=int, default=12)
    parser.add_argument("--questions-per-multi-context", type=int, default=8)
