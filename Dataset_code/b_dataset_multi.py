...
- 반드시 2개 이상의 문서를 실제로 사용한다.
- 한 문서만 보면 답할 수 있는 질문은 생성하지 않는다.
- 가능한 비교 유형:
  1. 값 비교
  2. 차이 비교
  3. 시점 비교
...
좋은 예:
- 두 계약 중 매출액 대비 값이 더 큰 계약은?
- 두 계약의 계약상대와 판매·공급지역은 각각 어떻게 다른가?
- 두 문서에 적용된 환율은 각각 얼마인가?
- 두 계약의 종료일은 어떻게 다른가?
...
def comparison_generation_prompt(
    bundle: ContextBundle,
    candidate_count: int,
) -> str:
    return f"""
다음 Context를 바탕으로 비교 QA 후보를 생성하라.
...
14. 비교에 여러 문서를 사용한 경우
문서별 evidence를 각각 별도 항목으로 작성한다.
...
최대 {candidate_count}개의 QA 후보를 생성하라.
""".strip()
...
- 질문과 답변은 반드시 제공된 Context에 직접 존재하는 정보만 사용한다.
- 일반 상식, 지리 지식, 산업 지식, 회사에 대한 외부 지식을 추가하지 않는다.
- Context에 없는 관계를 추론하지 않는다.

예를 들어 Context에

판매·공급지역: 미국

이라고만 되어 있다면 다음처럼 답하지 않는다.

- 미국은 북아메리카에 위치한다.
- 미국과 사우디아라비아는 지리적으로 멀리 떨어져 있다.
- 계약상대 회사가 미국 기업이다.

Context가 직접 보장하는 것은
'판매·공급지역이 미국'이라는 사실뿐이다.

또한 다음 두 필드를 서로 다른 의미로 취급한다.

계약상대: 회사명
판매·공급지역: 지역명

판매·공급지역을 계약상대의 소재국이나 국적으로 해석하지 않는다.
...
- answer는 반드시 Context에 직접 근거가 있는 정보만 사용한다.
- Context 밖의 일반 상식이나 외부 지식을 추가하지 않는다.
- 지리적 위치, 산업 특성, 회사 국적, 회사 소재지 등을 임의로 추론하지 않는다.
- 서로 다른 필드 사이의 관계를 임의로 만들어내지 않는다.

예:

Context:
계약상대: Hyundai Electric America Corporation
판매·공급지역: 미국

허용:
"계약상대는 Hyundai Electric America Corporation이고,
판매·공급지역은 미국입니다."

금지:
"Hyundai Electric America Corporation는 미국 회사입니다."
"미국은 북아메리카에 위치합니다."
"미국과 사우디아라비아는 지리적으로 멀리 떨어져 있습니다."

특히
'판매·공급지역'을 '계약상대의 국가/국적/소재지'로 해석하지 않는다.

질문 자체가 Context 밖 추론을 요구하면 해당 후보는 제거하거나
Context만으로 답할 수 있는 비교 질문으로 수정한다.
